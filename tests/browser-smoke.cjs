...
async function main() {
  ...
  try {
    ...
    const key = (type, modifiers = 0) => cdp('Input.dispatchKeyEvent', { type, key:'Alt', code:'AltLeft', windowsVirtualKeyCode:18, modifiers });
    const count = () => evaluate(`messages.filter(m => m.type === 'CAPTURE_STEP').length`);
    await click(80, 50); assert.equal(await count(), 0);
    await key('keyDown'); await click(80, 50, 1); await key('keyUp');
    await delay(650);
    ...
    await click(80, 110); await key('keyDown'); await click(80, 50, 1); await key('keyUp');
    await delay(100);
    assert.equal(await count(), 2, 'Alt works with an input focused');
    ...
    await key('keyDown'); await click(80, 50, 1); await key('keyUp'); assert.equal(await count(), 3);
    ...
    assert.equal(await count(), 3, 'Releasing Alt cancels capture');
    await key('keyDown');
    await delay(5100);
    await click(80, 50, 1);
    assert.equal(await count(), 3, 'Holding Alt beyond five seconds expires capture');
    await key('keyUp');
    await key('keyDown');
    await cdp('Input.dispatchMouseEvent', { type:'mousePressed', x:80, y:50, button:'left', clickCount:1, modifiers:1 });
    await delay(100);
    assert.equal(await count(), 4, 'Capture starts before the click is released');
    await cdp('Input.dispatchMouseEvent', { type:'mouseReleased', x:80, y:50, button:'left', clickCount:1, modifiers:1 });
    await delay(100);
    assert.equal(await count(), 4, 'Click does not duplicate pointer capture');
    await click(80, 50, 1);
    assert.equal(await count(), 5, 'Another click while Alt stays held captures again');
    await delay(100);
    await key('keyUp');
    console.log('PASS: release cancellation, five-second expiry, repeated captures per hold, early capture.');
    console.log('PASS: trusted Alt + click, plain/modifier clicks, typing, toolbar capture, hidden controls, save failure, pause.');
